...
- **Text-line animations** in `presentation_generator.py` must mirror PowerPoint-exported OOXML: `nodeType="clickEffect"` + `presetClass="entr"` + `grpId`, with `<p:set>` / `<p:animEffect>` as **direct** children of the clickEffect `childTnLst` (no extra nested `par` wrappers — Mac PowerPoint often ignores those). Do not insert a mainSeq “hide all at delay=0” boot step. Animations are only visible in PowerPoint **Slide Show**; rebuild the PPTX after animation code changes.

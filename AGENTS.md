...
- **File Triage** (`file_triage.py` / `file_triage_ui.py`) is a local inventory + decision tool, not an iCloud API. Scan a folder, sort by size, bucket files (Critical / Keep local / Archive / Delete), then Apply. Delete moves into `~/Documents-Local/_TriageTrash`. Demo tree: `python scripts/make_triage_demo.py`. Do not bind it to `0.0.0.0` — it can move files on disk.

...
| PRAL Checker | `python pral_ui.py` | http://127.0.0.1:5052 — food acidity from Remer & Manz multipliers. Desktop: `python desktop_pral.py`. CLI: `python pral.py food banana`. |
...
- **PRAL Checker** (`pral.py` / `pral_ui.py`) estimates food acid load from the five Remer & Manz multipliers (protein 0.49, phosphorus 0.037, potassium −0.021, magnesium −0.026, calcium −0.013). Positive = acid-forming, negative = alkaline-forming. Built-in foods supply typical per-100 g nutrients; the score is computed, not looked up. Default bind is localhost:5052. See `PRAL.md`.

...
- **PRAL Checker** (`pral.py` / `pral_ui.py`) estimates food acid load from the five Remer & Manz multipliers (protein 0.49, phosphorus 0.037, potassium −0.021, magnesium −0.026, calcium −0.013). Positive = acid-forming, negative = alkaline-forming. About 240 local foods live in `pral_foods.json`; **Search USDA** queries FoodData Central. Default bind is localhost:5052. On a Mac use `python3` or `./pral` from the repo folder — there is no `python` command. See `PRAL.md`.
